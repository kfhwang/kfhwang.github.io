/* @author K.F.Hwang kfhwang@nutc.edu.tw & 王鴻逸 @karta1172128
* Animator: 邱傳恩
* @version v1.0.0
...
window.onload = function () {
    ...
    window.Threshold = 600000; //distance matching threshold
    ...
    loadcanvas.style.zIndex = 200;
    loadcanvas.setAttribute("id", "loadCanvas");
    loadContext.canvas.width = vid_w;
    loadContext.canvas.height = vid_h;
    $("#cbox").append(loadcanvas);
    var size=100;
  
    loading = new RadialBar(loadContext, {
        x: (vid_w)/2+size/2,
        y: (vid_h)/2-size/2,
        angle: 360,
        title:"",
    ...
    xhr.open("GET", "js/data/Feature128.js", true);
    ...
    xhr.onprogress = function (e) {
        ...
        loadContext.clearRect(0, 0, vid_w, vid_h);
        loading.title=percent+"%";
        loading.set(percent);
        loading.update();
    };
    xhr.send();
    window.addEventListener("endProgress",function(){
        window.removeEventListener("endProgress", this);
            ...
            if(!notimer && timer>60){
                ...
                $("#dialog").on("dialogclose",function(){
                    if(navigator.userAgent.includes("iPhone")){
                        ...
                        $("#dialog>p").html("iPhone手機閒置過久後攝影機可能會進入休眠，需要重新載入網頁!");
